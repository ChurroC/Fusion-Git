...
def get_curve_data(curve: adsk.fusion.SketchCurve) -> Curve | Error:
    ...
    if curve_type == adsk.fusion.SketchLine.classType():
        ...
        line_curve_data: LineCurve = {
            "type": create_readable_value("Sketch line", cast(Literal["adsk::fusion::SketchLine"], curve_type)),
        ...
        circle_curve_data: CircleCurve = {
            "type": create_readable_value("Sketch Circle", cast(Literal["adsk::fusion::SketchCircle"], curve_type)),
...
def get_plane_data(plane: adsk.fusion.ConstructionPlane) -> Plane | Error:
    if plane.objectType == adsk.fusion.BRepFace.classType():
        # Sketch on surface
        face_plane_data: PlaneFace = {"type": create_readable_value("Face", "face")}
        ...
        base_plane_data: PlaneBase = {"type": create_readable_value("Base Plane", "base_plane"), "name": plane.name}
        return base_plane_data
    elif plane.timelineObject is not None:
        # Sketch on custom planes"
        custom_plane_data: PlaneCustom = {
            "type": create_readable_value("Custom Plane", "custom_plane"),
