...
def run(context):
    try:
        ...
        write_component_data_to_file(list(component_timeline.values())[0], src_folder_path)
...
def write_component_data_to_file(component_details: FusionComponentDetails, folder_path: str):
    data: Timeline = {
    ...
    for timeline_detail in timeline_details:
        data["features"].append(get_timeline_feature(timeline_detail, folder_path))
        ...
        if feature_type == adsk.fusion.Sketch.classType():
            ...
            if is_component_creation:
                write_component_data_to_file(
                    component_timeline[component.id],
                    os.path.join(folder_path, f"{feature.index}-{component.name}-{component.id}"),
                )
            elif is_linked:
                write_component_data_to_file(
                    component_timeline[component.id],
                    os.path.join(src_folder_path, "linked_component", f"{component.name}-{component.id}"),
                )
