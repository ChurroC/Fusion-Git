...
def set_extent_data(
    extrude_input: adsk.fusion.ExtrudeFeatureInput, extent_data: ExtrudeExtent | Error
    ...
    if extent_data["type"] == 0:  # OneSideFeatureExtentType
        ...
        extrude_input.setSymmetricExtent(
            adsk.core.ValueInput.createByString(extent_data["distance"]), bool(extent_data["isFullLength"])
        )
    else:
